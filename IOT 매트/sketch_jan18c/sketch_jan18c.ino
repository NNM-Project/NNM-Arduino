...
const char* ssid     = "iptime1"; //와이파이 이름
const char* password = "50255025"; //와이파이 비밀번호

const char* serverName = "http://192.168.0.9/pro/index.php"; //값정송 서버 도메인 주소

int countnum=0;
void setup() {
  // put your setup code here, to run once:
  Serial.begin(115200);//시리얼 정의
  //(로드셀 캘리브레이션 초기 값){
  ...
  if(reading-settingvalue1 < -16000){
    ...
    if(countnum>=6){
      if(WiFi.status()== WL_CONNECTED){
        ...
        httpRequestData = "input1=" + String(1);
